...
Game.run(320, 240) do
  ...
  s.render_text("Directions:", 8, 8,  font, color)
  directions = keys.select{|b| b.kind_of?(Symbol)}.map{|b|b.to_s}
  s.render_text(directions.join(","), 24, 24,  font, color)
  
  s.render_text("Buttons:", 8, 48, font, color)
  buttons = keys.select{|b| b.kind_of?(Integer)}.map{|b|b.to_s}
  s.render_text(buttons.join(","), 24, 64,  font, color)
end
