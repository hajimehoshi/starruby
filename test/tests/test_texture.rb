...
class TestTexture < Test::Unit::TestCase
  ...
  def test_load_io
    ...
    texture = open("images/ruby.png", "rb") do |io|
      Texture.load(io)
    end
  ...
  def test_load_io_cat
    ...
    data = open(path1, "rb"){|fp| fp.read} + open(path2, "rb"){|fp| fp.read}
  ...
  def test_render_text_hello_world
    if Font.exist?("Arial") and not RUBY_PLATFORM =~ /linux/ and not RUBY_PLATFORM =~ /darwin/
